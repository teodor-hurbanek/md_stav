        ...
        <div class="containerCustom">
            <div class="service-row">
                @foreach ($service->images as $key => $image)    
                    <div onclick="openGallery({{ $key }})" class="service-col shadow" style="background-image: url('{{ '/storage' . $image->filePath . $image->fileName }}');">
        ...
        <article onclick="closeModal()" class="gallery-wrapper">
            ...
            @foreach ($service->images as $image)    
                <div onclick="event.stopPropagation();" class="one-image">
                    <img src="{{ '/storage' . $image->filePath . $image->fileName}}" alt="Service image">
                </div>
            @endforeach
